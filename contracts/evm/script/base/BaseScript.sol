...
contract BaseScript is Script, Deployers {
    ...
    constructor() {
        ...
        if (block.chainid == 31337) {
            // Deploy fresh tokens on Anvil
            (currency0, currency1) = deployCurrencyPair();
            token0 = IERC20(Currency.unwrap(currency0));
            token1 = IERC20(Currency.unwrap(currency1));
        } else if (block.chainid == 84532) {
            // Base Sepolia: Deploy fresh mock tokens
            (currency0, currency1) = deployCurrencyPair();
            token0 = IERC20(Currency.unwrap(currency0));
            token1 = IERC20(Currency.unwrap(currency1));
        } else {
            token0 = IERC20(0x0165878A594ca255338adfa4d48449f69242Eb8F);
            token1 = IERC20(0xa513E6E4b8f2a923D98304ec87F64353C4D5C853);
