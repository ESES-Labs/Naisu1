...
export const COIN_USDC_CIRCLE =
  '0xa1ec7fc00a6f40db9693ad1415d0c193ad3906494428cf252621037bd7117e29::usdc::USDC'
export const COIN_USDC_CETUS =
 ...
 */
export async function getQuote(params: { poolId: string; amountIn: string; aToB: boolean }) {
...
export async function executeSwap(params: { poolId: string; amountIn: string; aToB: boolean }) {
...
export async function getPositions(owner: string) {
  const client = getClient()
  const positionType =
    '0x5372d555ac734e272659136c2a0cd3227f9b92de67c80dc11250307268af2db8::position::Position'
 ...
 */
export async function executeHarvest(params: { positionId: string }) {
  const keypair = getKeypair()
  const sender = keypair.toSuiAddress()
  const client = getClient()
 ...
 */
export async function buildHarvestTx(params: { positionId: string; userAddress: string }) {
  const client = getClient()
...
export async function buildZapTx(params: {
  ...
  const [, balBOut, receipt] = tx.moveCall({
    target: `${CETUS_CORE_PACKAGE}::pool::flash_swap`,
    arguments: [
      tx.object(CETUS_GLOBAL_CONFIG),
      tx.object(params.poolId),
 ...
 */
export async function getZapQuote(params: { poolId: string; amountIn: string }) {
  const pool = await getPoolInfo(params.poolId)
